...
export const login = async (email, password) => {
  // Neu dang co login process khac, doi no hoan thanh
  if (loginPromise) {
  ...
  try {
    ...
    // Xoa loginPromise sau khi hoan thanh (thanh cong hay that bai)
    loginPromise = null;
  }
};

// Ham thuc hien login that su
...
export const crawlChapterContent = async (url, retryCount = 0) => {
  ...
  try {
    ...
    console.log(
      "Using cookies:",
      sessionCookies ? sessionCookies.substring(0, 50) + "..." : "No cookies",
    );
    ...
    try {
      ...
      if (homeSetCookie) {
        const cfCookies = homeSetCookie
          .map((cookie) => cookie.split(";")[0])
          .join("; ");
        sessionCookies = sessionCookies
          ? `${sessionCookies}; ${cfCookies}`
          : cfCookies;
    ...
    if (response.status === 403) {
      ...
      throw new Error(
        "Website blocked request (403 Forbidden). Try using cookies from browser.",
      );
    }
    ...
    try {
      ...
      if (jsonLd) {
        ...
        if (data["@graph"]) {
          const chapterData = data["@graph"].find(
    ...
    if (contentDiv.length > 0) {
      ...
      console.log(
        "Content from .chap, paragraphs found:",
        contentDiv.find("p").length,
      );
    } else {
    ...
    const prevChapter = $(
      'a[rel="prev"], a.prev-chapter, a:contains("Chương trước"), a:contains("Chap trước")',
    )
      .first()
      .attr("href");
    const nextChapter = $(
      'a[rel="next"], a.next-chapter, a:contains("Chương sau"), a:contains("Chap sau")',
    )
      .first()
      .attr("href");
    ...
    if (is503Error(error) && retryCount === 0) {
      ...
      if (loginResult.success) {
        ...
        // Retry crawl chapter (chi retry 1 lan)
        return await crawlChapterContent(url, retryCount + 1);
      } else {
  ...
  try {
    ...
    try {
      ...
      if (jsonLd) {
        const data = JSON.parse(jsonLd);
        if (data["@graph"]) {
          const bookData = data["@graph"].find(
            (item) => item["@type"] === "Book",
          );
          if (bookData) {
            title = bookData.name || "";
    ...
    if (!author) {
      author = $('.author, .story-author, [class*="author"]')
        .first()
        .text()
        .trim();
    }
    if (!cover) {
      cover = $('.cover-img, .story-cover img, img[alt*="cover"]')
        .first()
        .attr("src");
    }

    const description = $(
      '.description, .story-description, .summary, [class*="description"]',
    )
      .first()
      .text()
      .trim();
    const genres = [];
    $('.genre, .category, .tag, [class*="genre"], [class*="category"]').each(
      (i, el) => {
    ...
    const views = $('.views, .view-count, [class*="view"]')
      .first()
      .text()
      .trim();
    const rating = $('.rating, .score, [class*="rating"]')
      .first()
      .text()
      .trim();
